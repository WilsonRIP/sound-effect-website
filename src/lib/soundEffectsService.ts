...
export const toSupabaseFormat = (
  sound: SoundEffect,
  userId: string
): Omit<SupabaseSoundEffect, "created_at"> => {
  return {
    id: sound.id,
    user_id: userId,
    name: sound.name,
    ...
    icon_content:
      sound.icon.type === "svg" && typeof sound.icon.content !== "string"
        ? "DEFAULT_SVG"
...
export const fromSupabaseFormat = (
  supabaseSound: SupabaseSoundEffect
): SoundEffect => {
  // Handle the SVG icon content
  let iconContent: string | React.ReactElement = supabaseSound.icon_content;

  if (
    supabaseSound.icon_type === "svg" &&
    supabaseSound.icon_content === "DEFAULT_SVG"
  ) {
...
export const fetchSoundEffects = async (): Promise<SoundEffect[]> => {
  ...
  console.log("Fetching sound effects for user ID:", userId);
  ...
  return data
    ? data.map((item: SupabaseSoundEffect) => fromSupabaseFormat(item))
    : [];
};
...
export const saveSoundEffects = async (
  sounds: SoundEffect[]
): Promise<boolean> => {
  const userId = await getUserId();
